...
class Driver {
  ...
  static waitToExpectElementWithOptions(
    locator: string,
    options: { has?: Locator; hasText?: string | RegExp }
  ) {
  ...
  /* This method wait to assert an element on the page, here element not necessarily to be locator */
  ...
  static waitToExpectElementUsingFrame(
    frameLocator: string,
  ...
  static async waitForElementToClickUsingFrame(
    frameLocator: string,
    elementLocator: string
  ) {
    expect(
      this.page.frameLocator(frameLocator).locator(elementLocator).click()
    );
  }

...
export default class NewEntityService extends Driver{
    ...
    public async SelectEntityFromGrid(entityName:string, clickBack2Entity:boolean){
    ...
    private async ClickAddEntityButton(){
         ...
         */
        await Driver.waitToExpectElement(newEntityPO.newEntityTextBox).toBeVisible();
        await Driver.waitToExpectElement(newEntityPO.modalCancelBtn).toBeEnabled();
        await Driver.waitToExpectElement(newEntityPO.modalCreateAndNewBtn).toBeDisabled();
    ...
    private async EnterEntityName(entityName:string){
        ...
        await Driver.waitToExpectElement(newEntityPO.modalCreateAndNewBtn).toBeEnabled();
    ...
    private async ClickEntityCreateButton(){
        ...
        await Driver.waitToExpectElement(newEntityPO.entitySuccessBackToEntity).toBeVisible();
        await Driver.waitToExpectElement(newEntityPO.addItemClass).toBeVisible();

    }

    private async ClickBackToEntity(){
    ...
    private async VerifyEntityIsShowingOnGrid(entityName:string){
        await Driver.waitToExpectElementWithOptions(newEntityPO.anchorTag, {hasText: entityName}).toBeVisible();

    }

    private async SelectEntity(entityName:string){
        await Driver.findElementWithOptions(newEntityPO.anchorTag,{hasText: entityName}).click();        
        await Driver.waitToExpectElement(newEntityPO.entitySuccessBackToEntity).toBeVisible();      
        await Driver.waitToExpectElement(newEntityPO.addItemClass).toBeVisible();
    }
}